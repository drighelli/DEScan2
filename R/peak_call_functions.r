...
findPeaks<-function(files, filetype="bam", chr=1:19, fraglen=200,
                      rlen=100, min_bin=50, min_win=1, max_win=20,
                      blocksize=10000, zthresh=5, min_count=0.1,
                      output_name="Peaks", save=TRUE, verbose=FALSE) {
        for (file in files) {
            ...
            if (filetype == "bed") {
                ...
                bed<-utils::read.table(file, sep="\t", header=FALSE)[, c(1:3, 6)] ## 6th column is not mandatory -> put a control
            ...
            while (TRUE) {
                ...
                cmat<-window_coverage(Fr=fr, Rr=rr, grid=grid0, fraglen=fraglen,
                                      rlen=rlen, max_win=max_win,
                                      min_win=min_win, verbose=FALSE)
                ...
                for (win in min_win:max_win) {
                    lam5k[, win - min_win + 1]<-c5k[offset5k +
                                                c(1:length(grid0)) -
                                                floor(win / 2)] * win / 5000
                    lam10k[, win - min_win + 1]<-c10k[offset10k +
                                                 c(1:length(grid0)) -
                ...
                z<-sqrt(2) * sign(cmat - lamloc) *
                    sqrt(cmat * log(pmax(cmat, min_count) / lamloc) -
                    (cmat - lamloc))
...
make_grid<-function(bed, fraglen, rlen, min_bin=10, min_rds=0,
    ...
    if (min_rds > 0) { # this portion of the code does not work!
        ...
        # coverage across all samples is less than min_rds.
...
read_bam<-function(file, chr) {
        ...
        if (file.exists(paste0(file,".bai")) == FALSE) { ## put a message
