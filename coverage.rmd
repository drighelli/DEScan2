...
runWinRleListSum <- IRanges::RleList(
...
wins <- minWinWidth:maxWinWidth
    runWinRleListMean <- IRanges::RleList(
                        lapply(wins, function(win) {
                            message("Running window ", win, " of ", maxWinWidth)
...
message("run sum on first col != 0", length(which(runWinRleListSum[[1]]!=0)))
message("run mean on first col != 0", length(which(runWinRleListMean[[1]]!=0)))
...
```{r}
    ...
    runWinRleList
    runWinMat=matrix(unlist(runWinRleList), ncol=maxWinWidth, byrow=FALSE)
```
...
```{r}
    ...
    s <- matrix(ncol = 3, nrow = 0, data = 0)
        ...
        for(i in 1:20) 
        {
            message("windi ", i, " != 0 ", length(which(as.vector(runWinRleList[[i]]) != 0)))
            message("cmati ", i, " != 0 ", length(which(cmat[,i] != 0)))
        }
        print(cor(as.vector(runWinRleList[[i]]), cmat[,i]))
```

Computing old lambdas and Z
...
computing new lambdas with run Mean
                                             ...
                                             verbose=TRUE)
{
 ...
 minCompRunWinRleList <- computeCoverageMovingWindowOnChr(
            ...
            Zm <- computeZ(lambdaChrRleList=lambdaChrRleList,
             ...
             length(Zm[which(Zm > 100, arr.ind=TRUE)])
```

computing new lambdas with run sum
